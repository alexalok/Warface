...
namespace Warface.Entities.Gamerooms
{
    public struct GameroomMission
    {
        ...
        GameroomMission(string key, string mode, string type, string modeName, string name, Difficulty difficulty)
        {
            Key = key;
            Mode = mode;
            Type = type;
        ...
        public static GameroomMission ParseNode(HtmlNode gameroomMissionNode)
        {
            ...
            return new GameroomMission(key, mode, type, modeName, name, difficulty);
        }
    }
}